...
// Base speed scales down with |error| instead of driving every
// corner at the same speed. error==0 (dead center) -> full speed
// (BASE_SPEED_MAX). |error| at or beyond ERROR_FULL_SCALE (the
// largest magnitude DetectLineFast can produce, since weights run
// -350..+350) -> BASE_SPEED_MIN. Linear ramp in between.
#define BASE_SPEED_MAX 180   // straight-line / near-zero-error speed
#define BASE_SPEED_MIN 150   // sharpest-correction speed
#define ERROR_FULL_SCALE 350 // matches max |weight| in DetectLineFast

static inline int16_t ComputeDynamicSpeed(int16_t error) __attribute__((always_inline));
static inline int16_t ComputeDynamicSpeed(int16_t error) {
  int16_t absErr = (error < 0) ? (int16_t)(-error) : error;
  if (absErr > ERROR_FULL_SCALE) absErr = ERROR_FULL_SCALE;
  int32_t speed = BASE_SPEED_MAX -
      (((int32_t)(BASE_SPEED_MAX - BASE_SPEED_MIN) * absErr) / ERROR_FULL_SCALE);
...
void RunPIDControl() {
  ...
  // Dynamic base speed: full speed when error is small/zero, down
  // to BASE_SPEED_MIN when the correction needed is large (sharp
  // corner), instead of always driving around a fixed MAX_SPEED.
  int16_t dynamicSpeed = ComputeDynamicSpeed(error);

  int16_t rightSpeed = dynamicSpeed + (int16_t)correction;
  int16_t leftSpeed  = dynamicSpeed - (int16_t)correction;
